...
#mongodb
client_mongo = MongoClient("mongodb+srv://aihack:aihack@cluster0.9tmj3me.mongodb.net/?retryWrites=true&w=majority")
db_mongo = client_mongo.Korok

store_nearby_locations = {}
...
@app.route('/api/places2', methods=['POST'])
def places_api2():
...
def places_api():
    query = {"location.coordinates": SON([("$near", [-122.40, 37.78]), ("$maxDistance", 1000)])}

    nearby_locations ={}

    docs = db_mongo.San_Francisco.find(query, {"name":1, "location":1, "_id":1}).limit(4)

    for doc in docs:
        #print(doc["name"])
        lgn, lat = doc['location']['coordinates']
        nearby_locations[doc['name']] = {'latitude': lat, 'longitude': lgn, 'db_path': str(doc['_id'])}
        store_nearby_locations[str(doc['_id'])] = {'latitude': lat, 'longitude': lgn, 'place_name': doc['name']}

    nearby_locations['user'] = {'latitude': 37.78, 'longitude': -122.40, 'db_path': ''}
...
def place_tapped():
    ...
    cafe = db_mongo.San_Francisco.find_one({"_id":ObjectId(place_name)});
    print(place_name);
    print(cafe);

    mec.places_dict = {place_name: cafe}
    #if user_id not in mec.convo.chat_histories.keys():
    print(f"creating chathistory for user {user_id}")
    mec.convo.chat_histories[user_id] = [{"role": "system", "content": f"Be an assistant and guide at {store_nearby_locations[place_name]['place_name']}." + wrapper.DEFAULT_PROMPT},]
    print("\n\n\n\n\n/////////")
